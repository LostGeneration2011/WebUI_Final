// Định nghĩa các biến giỏ hàng và số lượng giỏ hàng
let cart = {};
let cartCount = 0;

// Hàm xử lý sự kiện form đăng ký
...
// Hàm xử lý sự kiện form đăng nhập
...
// Hàm xử lý sự kiện nút "Đọc Thêm"
...
function displayCourses(page, courses) {
    const coursesPerPage = 5;
    ...
    attachCourseDetailEvents(courses);
}

// Hàm gắn sự kiện click cho các nút "Xem Chi Tiết" và "Mua Ngay"
function attachCourseDetailEvents(courses) {
...
// Gắn sự kiện khi document sẵn sàng
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Lấy danh sách khóa học từ API
    fetchCourses().then(courses => displayCourses(1, courses));
    ...
    document.getElementById('pagination').addEventListener('click', function(event) {
        if (event.target.classList.contains('page-link')) {
            ...
            fetchCourses().then(courses => displayCourses(page, courses));
        }
    });
